...
class Scheduler {
  ...
  async runMessage(message) {
    ...
    if (!target || !message?.enabled) return;
    ...
    try {
      ...
      this.observations.push({
        type: 'custom_message', messageId: message.id, text,
        accountId: target.accountId, channelId: target.channelId,
        timestamp: new Date().toISOString(), success: Boolean(result?.ok),
        error: result?.error || null
      });
      if (this.observations.length > 500) this.observations.splice(0, this.observations.length - 500);
    } catch (error) {
      this.observations.push({ type:'custom_message', messageId:message.id, text,
        timestamp:new Date().toISOString(), success:false, error:error.message });
    }
  }
  ...
  sync() {
    ...
    for (const [id, timer] of this.intervals) {
      if (!desired.has(id) || !desired.get(id).enabled) {
        clearInterval(timer);
  ...
  getStatus() {
    ...
    return {
      ...
      tasks: messages.map(m => ({
        id:m.id, text:m.text, intervalSeconds:m.intervalSeconds,
        enabled:m.enabled, running:this.intervals.has(m.id)
      })),
