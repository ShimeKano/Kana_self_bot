...
class Scheduler {
  ...
  async runMessage(message) {
    ...
    if (!target || !message.text?.trim()) return;
  ...
  sync() {
    ...
    for (const message of desired.values()) {
      ...
      // Wait for the configured interval before the first automatic send.
      const timer = setInterval(() => this.runMessage(message), message.intervalSeconds * 1000);
      this.intervals.set(message.id, timer);
    }
  }
  ...
  getStatus() {
    ...
    return {
      ...
      tasks: messages.map(m => ({
        ...
        enabled: m.enabled,
        running: this.intervals.has(m.id)
      })),
